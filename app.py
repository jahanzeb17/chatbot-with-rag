...
llm = ChatGroq(model="llama3-70b-8192", streaming=True)  # Enable streaming
...
def get_llm_response(retriever, query):
    ...
    persona_check = persona_check_chain.run(persona=st.session_state.persona, response=full_response)
    ...
    if "no" in persona_check.lower():
        ...
        full_response = style_adjust_chain.run(persona=st.session_state.persona, response=full_response)
